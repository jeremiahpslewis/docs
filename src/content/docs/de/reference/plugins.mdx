...
:::tip
Für den Test von jq-Abfragen bietet sich z. B. das Online-Tool [jqplay.org](https://jqplay.org/) an.
Für Regex-Tests eignet sich [regex101.com](https://regex101.com/).
:::
...
Der Parameter `method` bestimmt, welcher Wert gelesen wird: `power`, `energy`, `returnenergy` oder `soc`.
...
**Beispiel mit Batteriesteuerung** (`batterymode` verwendet Werte 1=normal, 2=hold, 3=charge, 4=holdcharge):
  ...
  set:
    ...
    switch:
      ...
      - case: 4 # holdcharge
        set:
          source: modbus
          # ... Modbus-Konfiguration für den Holdcharge-Modus
```

Alle Modi abdecken, die deine Batterie unterstützt, oder ein `default` ergänzen.
Ohne passenden Fall schlägt der Schreibvorgang fehl, die Batterie behält ihren bisherigen Modus und genau dieser bleibt auch über die API sichtbar.
