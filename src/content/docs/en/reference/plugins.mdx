...
The `method` parameter selects which value to read: `power`, `energy`, `returnenergy` or `soc`.
...
**Example with battery control** (`batterymode` uses values 1=normal, 2=hold, 3=charge, 4=holdcharge):
  ...
  set:
    ...
    switch:
      ...
      - case: 4 # holdcharge
        set:
          source: modbus
          # ... Modbus configuration for holdcharge mode
```

Cover every mode your battery supports, or add a `default`.
Without a matching case the write fails, the battery keeps its previous mode and that mode is also what stays visible on the API.
